...
static void mRTOS_JmpTask(struct TaskContext* TaskContextPtr){
   ATOMIC_BLOCK(ATOMIC_RESTORESTATE){
      asm volatile(
        ...
        "pop __tmp_reg__"               "\n\t"  // удалиь адрес возврата с вершины стека
        "pop __tmp_reg__"               "\n\t"
...
void mRTOS_Scheduler(void){
  uint16_t temp;
    ...
    if(--mRTOS_Tasks[mRTOS_CurrentTask].CurrentPriority == 0) // декремент текущего приоритета текущей задачи и если он равен нулю хотя бы для одной задачи, то восстановить приоритет всех задач
      for(mRTOS_Scheduler_i=0; mRTOS_Scheduler_i < mRTOS_InitTasksCounter; mRTOS_Scheduler_i++) // цикл восстановления приоритета всех задач
...
void mRTOS_SetSystemTime(uint32_t Time){
   ATOMIC_BLOCK(ATOMIC_RESTORESTATE){
     mRTOS_SystemTime = Time;           // прочитать значение системного времени
   }
}
...
uint32_t mRTOS_GetSystemTime(void){
  uint32_t temp;
   ATOMIC_BLOCK(ATOMIC_RESTORESTATE){
     temp = mRTOS_SystemTime;           // установить значение системного времени
   }
   return temp;
}
